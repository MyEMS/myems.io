...
- [Database Architecture Deployment](#database-architecture-deployment)
...
MyEMS (My Energy Management System) is an industry-leading open-source energy management system that complies with the ISO 50001 Energy Management System standard (GB/T 23331-2020). It is suitable for energy and carbon emission collection, analysis, and reporting in scenarios such as buildings, factories, shopping malls, hospitals, and parks.

### System module

MyEMS adopts a microservice architecture, which mainly includes the following modules:

- **myems-api**: RESTful API service (Python + Falcon)
- **myems-admin**: Admin backend UI (AngularJS)
- **myems-web**: User Web interface (ReactJS)
- **myems-modbus-tcp**: Modbus TCP data collection service
- **myems-normalization**: Data normalization service
- **myems-cleaning**: Data cleaning service
- **myems-aggregation**: Data aggregation service (energy consumption, billing, carbon emission calculation)

### Database Overview

MyEMS adopts a multi database separation architecture, which stores different types of data in different databases to improve performance, facilitate maintenance and expansion. The system includes **13 databases**:

|Database Name|purpose|Main functions|Data scale|
|-----------|------|----------|---------|
|`myems_system_db`|System configuration database|Basic data such as storage system configuration, device information, user permissions, etc|small and medium-sized|
|`myems_historical_db`|Historical data database|Store real-time monitoring data, historical data, cost files, etc|**Large scale**|
|`myems_energy_db`|Energy consumption database|Store energy consumption statistics data for various devices (by hour, day, month, year)|**Large scale**|
|`myems_billing_db`|Billing database|Store energy consumption data related to billing|**Large scale**|
|`myems_carbon_db`|Carbon Emissions Database|Store energy consumption data related to carbon emissions|**Large scale**|
|`myems_energy_baseline_db`|Energy consumption baseline database|Store energy consumption baseline data for energy-saving analysis|medium-sized|
|`myems_energy_model_db`|Energy consumption model database|Store 8760 hours of annual energy consumption model data|medium-sized|
|`myems_energy_plan_db`|Energy consumption plan database|Storage energy consumption plan and target data|medium-sized|
|`myems_energy_prediction_db`|Energy consumption prediction database|Storage energy consumption prediction data|medium-sized|
|`myems_fdd_db`|Fault diagnosis database|Store data related to fault detection and diagnosis|medium-sized|
|`myems_user_db`|User database|Store user authentication, API keys, email messages, etc|small|
|`myems_reporting_db`|Report database|Store email messages and attachments related to reports|small|
|`myems_production_db`|production database|Store product data related to production|small|

---

## Database architecture deployment

Install database architecture and scripts for MyEMS.

### Prerequisite conditions

MyEMS can run on the following MySQL compatible database servers.

[MySQL 8.0 (64 bit) and above](https://www.mysql.com/)

[MariaDB 10.5 (64 bit) and above versions](https://mariadb.org/)

[SingleStore 7.0 and above](https://www.singlestore.com/)

### Deployment

Execute the following script in the MySQL command line, or in other MySQL client tools such as MySQL Workbench, Navicat, DBaver, php MyAdmin, etc.
...
The demonstration data is optional. To insert demonstration data, execute the following script:
...
::: tip How to change COLLATE on MySQL servers before version 8.0
...
::: tip How to fix MySQL error "A packet larger than 'max_allowed_pack'
bytes"

Change the my.cnf or my.ini file under the mysqld section (usually found in/etc/mysql/) and set:
...
Do not install databases into Docker containers in production environments
...
- **Use UTC for All Times**: Both database storage and API responses use UTC time
- **Frontend Conversion**: Frontend is responsible for converting to local time for display
- **Time Format**: Use `DATETIME` type, format: `YYYY-MM-DD HH:MM:SS`
- **Timezone Issues**: Pay attention to daylight saving time and timezone conversions

### 2. Data Type Selection

- **Numerical Values**: Use `DECIMAL(21, 6)` to ensure precision and avoid floating-point errors
- **Text**: Use `VARCHAR` for short text, `TEXT` or `LONGTEXT` for long text
- **JSON**: Use `LONGTEXT` to store JSON strings
- **Binary**: Use `LONGBLOB` to store files

### 3. Query Optimization

- **Use Indexes**: Query conditions should preferably use indexed fields
- **Avoid Full Table Scans**: Avoid `SELECT *` for large tables
- **Pagination**: List queries must be paginated to avoid returning large amounts of data at once
- **Time Range**: Historical data queries must limit the time range

### 4. Transaction Handling

- **Configuration Data**: System configuration tables use transactions to ensure consistency
- **Historical Data**: Historical data tables typically do not use transactions to improve write performance
- **Batch Operations**: Use transactions for batch inserts, rollback on failure

### 5. Data Consistency

- **Foreign Key Constraints**: System configuration tables use foreign keys to ensure data consistency
- **Join Queries**: Use JOIN queries for related data, avoid multiple separate queries
- **Data Validation**: Validate data format and range before writing

### 6. Performance Optimization

- **Read-Write Separation**: Historical data tables support read-write separation
- **Partitioned Tables**: Consider time-based partitioning for large tables
- **Caching Strategy**: Use caching for configuration data and latest values
- **Batch Operations**: Use `INSERT ... VALUES (...), (...), (...)` for batch inserts

### 7. Security Considerations

- **SQL Injection**: Use parameterized queries to avoid SQL injection
- **Password Encryption**: User passwords use salt + hash, never store plain text
- **Access Control**: API interfaces require authentication
- **Data Masking**: Sensitive data (passwords, keys) should not be logged

### 8. Error Handling

- **Exception Catching**: Database operations need exception handling
- **Error Logging**: Record detailed error information for troubleshooting
- **Retry Mechanism**: Support retries for network errors
- **Fallback Strategy**: Provide fallback solutions when services are unavailable
...
Recommended database installation order:

1. **myems_system_db** - System configuration database (must be installed first)
...
6. **myems_carbon_db** - Carbon emission database
...
```bash
# Navigate to database installation directory
cd database/install
